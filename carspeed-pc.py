...
if (setup_complete == False):
    ...
    while not setup_complete:
        cv2.imshow("Speed Camera",org_image)
...
while (True)
    ...
    if background is None:
        ...
        background = background[upper_left_y:lower_right_y,upper_left_x:lower_right_x]
        background = cv2.GaussianBlur(background, BLURSIZE, 0)
                #gray.copy().astype("float")
        lastTime = timestamp
    ...
    # white
    diff = cv2.absdiff(background, gray) # cv2.convertScaleAbs(base_image)
    diff = cv2.threshold(diff, THRESHOLD, 255, cv2.THRESH_BINARY)[1]
    
    # dilate the thresholded image to fill in any holes, then find contours
    # on thresholded image
    diff = cv2.dilate(thresh, es, iterations=2)
    image2, cnts, hierarchy = cv2.findContours(diff.copy(), cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
